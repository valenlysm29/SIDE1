...
const input = path.join(root, 'tools/model-sources/mona/mona.original.glb');
const output = path.join(root, 'assets/models3d/npcs/mona.glb');
...
(async () => {
  ...
  const [simplified, error] = MeshoptSimplifier.simplifyWithAttributes(indices, positions, 3, attributes, 5, [.5, .5, .5, 2, 2], null, 150000, .006);
  ...
  gltf.asset.generator = 'SIDE Mona optimization / meshoptimizer 0.25.0';
  gltf.nodes[0].name = 'Mona'; gltf.meshes[0].name = 'Mona';
  ...
  const result = Buffer.concat([header,json,binHeader,binary]);fs.writeFileSync(output,result);
  const report = {source:'tools/model-sources/mona/mona.original.glb',output:'assets/models3d/npcs/mona.glb',sourceSha256:crypto.createHash('sha256').update(source).digest('hex'),outputSha256:crypto.createHash('sha256').update(result).digest('hex'),sourceBytes:source.length,outputBytes:result.length,sourceTriangles:indices.length/3,triangles:simplified.length/3,vertices:vertexCount,relativeError:error,animations:0,rigged:false,textures:'Color 2048px; normal and metallic/roughness 1024px',note:'Original supplied by the user. No new license is assigned to the model.'};
  fs.writeFileSync(path.join(root,'tools/model-sources/mona/manifest.json'),JSON.stringify(report,null,2)+'\n');console.log(report);
