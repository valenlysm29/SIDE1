...
assert.equal(await page.locator('[data-single-store]').count(),1);assert.equal(await page.locator('[data-store-qty],[data-store-step]').count(),0);
await page.locator('#storeDistrict').selectOption('miraflores');
...
console.log('PASS one store, district, web channel and save');
