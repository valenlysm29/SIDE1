...
function openTeacherPanel(){closeModal();window.location.href='docente.html?v=20260909-2'}
...
async function openStudentTutorial(){
  ...
  if(!mount.dataset.loaded){try{const response=await fetch('tutorial.html?v=20260909-2');if(!response.ok)throw new Error('No se pudo cargar tutorial.html');mount.innerHTML=await response.text();mount.dataset.loaded='1'}catch(error){console.error(error);showScreen('studentLobby');return}}
  ...
  const diagrams=lines.length?`<article class="dop-product" aria-labelledby="dopProductTitle">
    ...
    <div class="dop-industrial">
      ...
      <ol class="dop-main-line">
        ...
        <li class="dop-operation-row dop-combined-row"><aside class="dop-side-input dop-secondary-input"><span>MATERIA PRIMA SECUNDARIA</span><strong>Accesorios · ${escapeHtml(selection('ACCESORIOS'))}</strong><small>Necesario: ${material('ACCESORIOS')?.neededForTarget.toLocaleString('es-PE')||0} u. · disponible: ${material('ACCESORIOS')?.available.toLocaleString('es-PE')||0} u.${provider?` · ${escapeHtml(provider.label)}`:''}</small></aside><i class="dop-symbol combined"><span>1</span></i><div class="dop-operation-copy"><strong>Colocación e inspección de accesorios</strong><span>${plan.producibleUnits.toLocaleString('es-PE')} bolsos intervenidos</span><small>Actividad combinada: se colocan y verifican cierres, hebillas, broches y demás accesorios.</small></div></li>
