...
function sectionReady(cat=currentCategory){const c=categoryByCat(cat);if(!c)return false;return c.items.filter(i=>i.type!=='info'&&itemRequired(i)).every(itemComplete)}
...
function bindDecisionControls(){
  ...
  document.querySelectorAll('[data-step]').forEach(b=>b.addEventListener('click',()=>{if(decisionEditingBlocked())return;changeQty(b.dataset.step,b.dataset.option,Number(b.dataset.delta));}));
  document.querySelectorAll('[data-qty]').forEach(i=>i.addEventListener('input',()=>{if(decisionEditingBlocked())return;const d=initDraft(findDecisionItem(i.dataset.qty));d.quantities[i.dataset.option]=Math.max(0,Number(i.value)||0);updateSectionCost();updateRowCost(i.dataset.qty)}));
  document.querySelectorAll('[data-number-step]').forEach(b=>b.addEventListener('click',()=>{if(decisionEditingBlocked())return;const item=findDecisionItem(b.dataset.numberStep),d=initDraft(item);d.value=Math.max(Number(item.min||0),Number(d.value||0)+Number(b.dataset.delta));renderDecisionCategory()}));
  document.querySelectorAll('[data-number]').forEach(i=>{
    i.addEventListener('input',()=>{if(decisionEditingBlocked())return;const item=findDecisionItem(i.dataset.number),d=initDraft(item);const raw=i.value;d.value=raw===''?'':Math.max(Number(item.min||0),Number(raw)||0);updateSectionCost();});
    i.addEventListener('change',()=>{if(decisionEditingBlocked())return;const item=findDecisionItem(i.dataset.number),d=initDraft(item);d.value=Math.max(Number(item.min||0),Number(i.value)||0);updateSectionCost();updateRowCost(i.dataset.number);});
  });
  document.querySelectorAll('[data-staff-step]').forEach(b=>b.addEventListener('click',()=>{if(decisionEditingBlocked())return;changeStaff(b.dataset.staffStep,b.dataset.store,Number(b.dataset.delta));}));
  document.querySelectorAll('[data-staff]').forEach(i=>i.addEventListener('input',()=>{if(decisionEditingBlocked())return;const d=initDraft(findDecisionItem(i.dataset.staff));d.staff[i.dataset.store]=Math.max(0,Number(i.value)||0);updateSectionCost();updateRowCost(i.dataset.staff)}));
  document.querySelectorAll('[data-loan]').forEach(i=>i.addEventListener('input',()=>{if(decisionEditingBlocked())return;setLoan(i.dataset.loan,Number(i.value),true);}));
  document.querySelectorAll('[data-loan-number]').forEach(i=>i.addEventListener('input',()=>{if(decisionEditingBlocked())return;setLoan(i.dataset.loan,Number(i.value),true);}));
}
...
function saveCurrentSection(){
  ...
  const movement=next-old;animateCash(movement);decisionDrafts={};renderTabs();renderDecisionCategory();updateHud();toast('Borrador guardado. Puedes seguir revisándolo hasta presionar ENVIAR DECISIONES.');
}
...
function sendCurrentSection(){
  ...
  saveCurrentSection();
  if(!sectionReady(currentCategory)){toast('Completa y guarda las decisiones obligatorias de esta pestaña antes de enviarla.');return}
...
function submitAllDecisions(){
  ...
  if(decisionProgressPercent()!==100){toast('Completa y guarda todas las decisiones obligatorias antes de enviar todo.');return}
