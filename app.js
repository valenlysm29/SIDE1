...
function openTeacherPanel(){closeModal();window.location.href='docente.html?v=20260909-3'}
...
async function openStudentTutorial(){
  ...
  if(!mount.dataset.loaded){try{const response=await fetch('tutorial.html?v=20260909-3');if(!response.ok)throw new Error('No se pudo cargar tutorial.html');mount.innerHTML=await response.text();mount.dataset.loaded='1'}catch(error){console.error(error);showScreen('studentLobby');return}}
  ...
  const diagrams=lines.length?`<article class="dop-product" aria-labelledby="dopProductTitle">
    ...
    <div class="dop-industrial">
      ...
      <ol class="dop-main-line">
        <li class="dop-operation-row"><div class="dop-side-space"></div><i class="dop-symbol operation"><span>1</span></i><div class="dop-operation-copy"><strong>Corte de piezas</strong><span>${plan.producibleUnits.toLocaleString('es-PE')} juegos de piezas cortadas</span><small>${cut.staff} operario(s) · ${cut.machines} mesa(s) · capacidad total ${cut.cycleCapacity.toLocaleString('es-PE')} u./ciclo</small></div></li>
        <li class="dop-operation-row"><aside class="dop-side-input dop-auxiliary-input"><span>INSUMO AUXILIAR</span><strong>Hilo · ${escapeHtml(selection('HILO'))}</strong><small>Necesario: ${material('HILO')?.neededForTarget.toLocaleString('es-PE')||0} m · disponible: ${material('HILO')?.available.toLocaleString('es-PE')||0} m</small></aside><i class="dop-symbol operation"><span>2</span></i><div class="dop-operation-copy"><strong>Armado, ensamblado y costura</strong><span>${plan.producibleUnits.toLocaleString('es-PE')} cuerpos de bolso ensamblados</span><small>${assembly.staff} operario(s) · ${assembly.machines} máquina(s) · capacidad total ${assembly.cycleCapacity.toLocaleString('es-PE')} u./ciclo</small></div></li>
        <li class="dop-operation-row dop-combined-row"><aside class="dop-secondary-branch"><div class="dop-branch-material"><span>MATERIA PRIMA SECUNDARIA</span><strong>Accesorios · ${escapeHtml(selection('ACCESORIOS'))}</strong><small>Necesario: ${material('ACCESORIOS')?.neededForTarget.toLocaleString('es-PE')||0} u. · disponible: ${material('ACCESORIOS')?.available.toLocaleString('es-PE')||0} u.${provider?` · ${escapeHtml(provider.label)}`:''}</small></div><div class="dop-branch-check"><i class="dop-symbol inspection"><span>1</span></i><strong>Selección y verificación</strong></div></aside><i class="dop-symbol combined"><span>1</span></i><div class="dop-operation-copy"><strong>Colocación e inspección de accesorios</strong><span>${plan.producibleUnits.toLocaleString('es-PE')} bolsos intervenidos</span><small>El ramal secundario se une aquí: se colocan y verifican cierres, hebillas, broches y demás accesorios.</small></div></li>
        <li class="dop-operation-row"><div class="dop-side-space"></div><i class="dop-symbol operation"><span>3</span></i><div class="dop-operation-copy"><strong>Acabado final</strong><span>${plan.producibleUnits.toLocaleString('es-PE')} bolsos acabados</span><small>${finish.staff} operario(s) · ${finish.machines} máquina(s) · capacidad total ${finish.cycleCapacity.toLocaleString('es-PE')} u./ciclo</small></div></li>
        <li class="dop-operation-row dop-inspection-row"><aside class="dop-side-input dop-quality-data"><span>CONTROL DE PRODUCTO TERMINADO</span><strong>${productWarranty?escapeHtml(productWarranty.label):'Sin cobertura seleccionada'}</strong><small>Revisión visual, costuras, resistencia, medidas y funcionamiento antes del catálogo.</small></aside><i class="dop-symbol inspection"><span>1</span></i><div class="dop-operation-copy"><strong>Inspección final del bolso</strong><span>${plan.producibleUnits.toLocaleString('es-PE')} unidades conformes</span><small>Solo los bolsos aprobados pasan a producción final y al catálogo productivo.</small></div></li>
      </ol>
