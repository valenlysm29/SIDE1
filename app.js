...
function openTeacherPanel(){closeModal();window.location.href='docente.html?v=20260909-4'}
...
async function openStudentTutorial(){
  ...
  if(!mount.dataset.loaded){try{const response=await fetch('tutorial.html?v=20260909-4');if(!response.ok)throw new Error('No se pudo cargar tutorial.html');mount.innerHTML=await response.text();mount.dataset.loaded='1'}catch(error){console.error(error);showScreen('studentLobby');return}}
  ...
  const diagrams=lines.length?`<article class="dop-product" aria-labelledby="dopProductTitle">
    ...
    <div class="dop-industrial">
      ...
      <ol class="dop-main-line">
        ...
        <li class="dop-operation-row dop-combined-row"><aside class="dop-secondary-branch"><div class="dop-branch-material"><span>MATERIA PRIMA SECUNDARIA</span><strong>Accesorios</strong><small>${escapeHtml(selection('ACCESORIOS'))}</small></div><div class="dop-branch-check"><i class="dop-symbol inspection"><span>1</span></i><strong>Clasificación</strong></div></aside><i class="dop-symbol combined"><span>1</span></i><div class="dop-operation-copy"><strong>Colocación de accesorios</strong><span>${plan.producibleUnits.toLocaleString('es-PE')} bolsos</span><small>Operación e inspección combinadas.</small></div></li>
