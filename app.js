...
function openTeacherPanel(){closeModal();window.location.href='docente.html?v=20260909-7'}
...
async function openStudentTutorial(){
  ...
  if(!mount.dataset.loaded){try{const response=await fetch('tutorial.html?v=20260909-7');if(!response.ok)throw new Error('No se pudo cargar tutorial.html');mount.innerHTML=await response.text();mount.dataset.loaded='1'}catch(error){console.error(error);showScreen('studentLobby');return}}
  ...
  const diagrams=lines.length?`<article class="dop-product" aria-labelledby="dopProductTitle">
    ...
    <div class="dop-industrial">
      <div class="dop-process-map">
        ...
        <div class="dop-map-step dop-map-main-step dop-map-finish"><i class="dop-symbol operation"><span>3</span></i><div><strong>Acabado final</strong><span>${plan.producibleUnits.toLocaleString('es-PE')} bolsos acabados</span><small>${finish.staff} operario(s) · ${finish.machines} máquina(s) · ${finish.cycleCapacity.toLocaleString('es-PE')} u./ciclo</small></div></div>
        <div class="dop-map-step dop-map-main-step dop-map-final-check"><i class="dop-symbol inspection"><span>2</span></i><div><strong>Inspección final</strong><span>${plan.producibleUnits.toLocaleString('es-PE')} unidades conformes</span><small>${productWarranty?escapeHtml(productWarranty.label):'Control de producto terminado'}</small></div></div>
      </div>
