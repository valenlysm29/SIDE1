...
(() => {
  ...
  function loadBusinessState() {
    ...
    businessState.enterprise ||= { machinesInstalled: 0, unlockedStores: [], productionLevel: 1, inventoryVisual: true };
    businessState.enterprise.unlockedStores ||= [];
  ...
  // Integración empresa -> mundo 3D: mantiene visibles las consecuencias de decisiones.
  function syncEnterpriseState() {
    if (!businessState) return;
    const enterprise = businessState.enterprise || (businessState.enterprise = {});
    enterprise.machinesInstalled = Math.max(0, owned('ENSAMBLE') + owned('ACABADOS') + owned('CORTE'));
    enterprise.unlockedStores = enterprise.unlockedStores || [];
    enterprise.productionLevel = Math.max(1, Math.round(1 + enterprise.machinesInstalled * 0.15));
    saveBusinessState();
  }
  ...
  // V4: Planta empresarial conectada al estado del simulador.
  // Genera elementos visibles según decisiones tomadas.
  function buildEnterpriseFactory() {
    if (!dynamicGroup || !THREE) return;
    const factory = new THREE.Group();
    factory.name = 'enterpriseFactoryZone';

    const area = box(22, 0.05, 7, 0x5a6268, 0, 0.03, -2.2, 0.55, 0.01);
    factory.add(area);

    // Separación visual: almacén -> producción -> oficina
    factory.add(addTextLabel('ALMACÉN', -7, 1.9, -2.2, '#7dffb2', 0.55, 'rgba(0,0,0,.55)'));
    factory.add(addTextLabel('PRODUCCIÓN', 0, 1.9, -2.2, '#ffd86b', 0.55, 'rgba(0,0,0,.55)'));
    factory.add(addTextLabel('OFICINA', 7, 1.9, -2.2, '#9ed6ff', 0.55, 'rgba(0,0,0,.55)'));

    // Cajas de almacén visibles según inventario.
    const reserve = totalReserveStock ? totalReserveStock() : 0;
    for (let i = 0; i < Math.min(12, Math.ceil(reserve / 10)); i++) {
      factory.add(box(.55,.45,.55,0x8a6748,-8 + (i%4)*.75,.23,-3.1 + Math.floor(i/4)*.7,.72,.02));
    }

    // Máquinas visibles según decisiones.
    const machineCount = Number(businessState?.enterprise?.machinesInstalled || 0);
    for (let i = 0; i < Math.min(machineCount, 8); i++) {
      const machine = makeMachine(-2.8 + (i%4)*1.6, -2.8 + Math.floor(i/4)*1.8, i%2 ? 'cut' : 'assembly', i > 4 ? 3 : 2);
      factory.add(machine);
    }

    // Oficina física.
    factory.add(box(3,1.4,2.2,0x34414d,7,0.7,-2.2,.35,.04));
    factory.add(box(.9,.5,.05,0x111924,7,1.65,-3.35,.15,.25));

    dynamicGroup.add(factory);
  }

  function refreshEnterpriseFactory() {
    // Limpieza simple: reconstruye elementos visuales en cada actualización.
    buildEnterpriseFactory();
  }
